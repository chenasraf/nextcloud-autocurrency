...
class FetchCurrenciesService {
	...
	private function processCurrency(Currency $currency, object $project, string $baseCurrency): void {
		...
		// Check if this currency has a custom configuration
		$customCurrency = $this->findCustomCurrency($lcur);
		$source = null;

		if ($customCurrency !== null) {
			$this->logger->info("Using custom currency configuration for $lcur");
			$result = $this->fetchCustomCurrencyRate($customCurrency, $baseCurrency);

			if ($result === null) {
				$this->logger->warning("Failed to fetch custom rate for $lcur, falling back to standard API");
		...
		if ($newRate === null) {
			$this->logger->error("Failed to fetch rate for $lcur");
			return;
		}
	...
	private function fetchCustomCurrencyRate(CustomCurrency $customCurrency, string $baseCurrency): ?array {
		try {
			...
			$response = $this->fetchApiResponse($endpoint);
	...
	private function convertFromUsdToBase(float $usdRate, string $baseCurrency): float {
		try {
			...
			// then 1 BaseCurrency = (1/Y) USD = (1/Y) * (1/X) CustomCurrency
			// So: rate in base = (1 / usdToBase) / usdRate = 1 / (usdToBase * usdRate)
			// But we store as 1/baseRate, so we need: usdRate / usdToBase

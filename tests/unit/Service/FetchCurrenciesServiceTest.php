...
final class CurrencyResolverTest extends TestCase {
	...
	/**
	 * Test USD conversion with very small conversion rate
	 */
	public function testUsdConversionWithVerySmallConversionRate(): void {
		$customCurrency = new CustomCurrency();
		$customCurrency->setCode('BTC');
		$customCurrency->setApiEndpoint('https://api.example.com/btc');
		$customCurrency->setJsonPath('$.usd');
		...
		$apiCache->setValue($this->resolver, [
			...
			'usd' => [
				'usd' => [
					'satoshi' => 0.00000001, // Very small conversion rate
				],
			],
		]);

		$result = $method->invoke($this->resolver, $customCurrency, 'satoshi');

		$this->assertIsArray($result);
		// 50000 / 0.00000001 = 5,000,000,000,000
		$expectedRate = 50000 / 0.00000001;
